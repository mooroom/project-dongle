...
function Guide({type}) {
  ...
  if (type === "learn") {
    guideText = "동글이에게 얼굴, 팔, 다리 사진을 찍어서 보여주면 동글이가 얼굴, 팔, 다리를 만들 수 있어!"
    guideType = types.learn;
  } else if (type === "arm") {
    guideText = "손과 팔 사진 8장을 찍어서 모아줘! 손과 팔만 잘 나오도록 찍어줘야해~"
    guideType = types.arm;
  } else if (type === "face") {
    guideText = "얼굴 사진 8장을 찍어서 모아줘! 얼굴 전체가 다 나오게 찍어줘야해~"
    ...
    <GuideBlock>
      ...
      <Card>
        ...
        {guideType.map(content => (
          <CardImg>
            <img className="tag" src={content[0]}/>
            <div className="left">{content[1]}</div>
            <img className="right" src={content[2]}/>
          </CardImg>
        ))}
